...
fn bounded_javascript_syntax_retains_each_called_grouped_import_binding() {
    for import in [
        ...
        "import /* 'decoy' */ {frequent, occasional}\nfrom './zzz_provider';",
    ] {
        let mut paths = Vec::new();
        let mut sources = Vec::new();
        for index in 0..254 {
        ...
        let fixture =
            ProjectFixture::new_owned(paths, sources, SemanticProjectLanguage::JavaScript);
        ...
        let callee = output
            ...
            .iter()
            .find(|entity| {
                entity.kind == EntityKind::Function && entity.display_name == "occasional"
            })
        ...
        let secondary_caller = output
            ...
            .find(|entity| {
                entity.kind == EntityKind::Function
                    && entity.display_name == "secondaryFrequentCaller"
            })
        ...
        assert!(output.document().diagnostics.iter().any(|diagnostic| {
            diagnostic.code.as_str() == PROJECT_SYNTAX_FACT_LIMIT_DIAGNOSTIC
        }));
        assert!(output.document().occurrences.iter().any(|occurrence| {
            occurrence.role == OccurrenceRole::CallSite
                && occurrence.enclosing == Some(caller.id)

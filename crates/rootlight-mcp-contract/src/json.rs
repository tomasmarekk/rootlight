...
pub(crate) const MAX_SUPPORTED_JSON_DEPTH: usize = 64;

/// Explicit structural limits for one untrusted JSON value.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct JsonLimits {
    /// Maximum object or array nesting below the top-level value.
    pub max_depth: usize,
    /// Maximum decoded UTF-8 bytes in any string or object key.
    pub max_string_bytes: usize,
    /// Maximum properties in one object.
    pub max_object_properties: usize,
    /// Maximum values in one array.
    pub max_array_items: usize,
    /// Maximum aggregate values in the complete document.
    pub max_nodes: usize,
}

/// A well-formed JSON value rejected by a structural safety rule.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum JsonIssue {
    /// One explicit structural limit was exceeded.
    Limits,
    /// An object repeated a member name.
    DuplicateName,
}

/// Failure to decode one bounded untrusted JSON value.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ParseFailure {
    /// The input is not one complete JSON value.
    Malformed,
    /// The input is JSON but violates a structural safety rule.
    Rejected(JsonIssue),
    /// A bounded allocation could not be reserved.
    MemoryUnavailable,
}

/// Decodes exactly one JSON value under the supplied structural limits.
///
/// # Errors
///
/// Returns [`ParseFailure`] for malformed, duplicate, oversized, overly
/// nested, or allocation-failing input.
pub fn parse_bounded(input: &[u8], limits: JsonLimits) -> Result<Value, ParseFailure> {
...
/// Test support that also reports bounded array reservation growth events.
///
/// # Errors
///
/// Returns the same [`ParseFailure`] values as [`parse_bounded`].
#[doc(hidden)]
pub fn parse_bounded_with_array_growths(
    input: &[u8],
    limits: JsonLimits,
...
impl ParseState {
    ...
    fn reserve_array<E: serde::de::Error>(
        ...
        let grew = match try_reserve_bounded(values, additional, self.limits.max_array_items) {
            Ok(grew) => grew,

//! Versioned common normalized fact documents for Rootlight.
//!
...
/// The baseline normalized fact-document version.
pub const NORMALIZED_IR_VERSION: IrVersion = IrVersion::new(1, 1);
/// The normalized fact-document version admitting stylesheet entities.
...
/// The normalized fact-document version admitting document sections and link definitions.
pub const NORMALIZED_IR_VERSION_V1_5: IrVersion = IrVersion::new(1, 5);
...
/// Exact supported normalized versions; producers select the minimum required.
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub enum NormalizedIrVersion {
    /// Baseline common facts, without stylesheet rule or keyframes entities.
    #[default]
    V1_1,
    /// Common facts including stylesheet rules and named keyframes.
    V1_2,
    ...
    V1_4,
    /// Common facts including authored document sections and named link definitions.
    V1_5,
}
...
impl NormalizedIrVersion {
    ...
    pub const fn value(self) -> IrVersion {
        match self {
            ...
            Self::V1_5 => NORMALIZED_IR_VERSION_V1_5,
        }
    }
}
...
impl<'de> Deserialize<'de> for NormalizedIrVersion {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        match version {
            ...
            NORMALIZED_IR_VERSION_V1_5 => Ok(Self::V1_5),
            version => Err(de::Error::custom(format_args!(
                "expected normalized IR version 1.1, 1.2, 1.3, 1.4 or 1.5, got {}.{}",
                version.major(),
                version.minor()
            ))),
        }
    ...
    fn json_schema(_generator: &mut schemars::SchemaGenerator) -> schemars::Schema {
        schemars::json_schema!({
            ...
            "properties": {
                ...
                "minor": { "type": "integer", "enum": [1, 2, 3, 4, 5] }
            },
...
pub enum EntityKind {
    ...
    /// A configuration key.
    ConfigurationKey,
    /// A stylesheet rule associating a selector with declarations.
    StyleRule,
    /// A named stylesheet keyframe animation, not a callable function.
    Keyframes,
    ...
    ExternalSymbol,
    /// A source markup element, not an inferred browser DOM node or class.
    MarkupElement,
    ...
    Modifier,
    /// A source-authored document section introduced by a heading, not a language module.
    DocumentSection,
    /// A named source binding for a link destination, not the destination resource itself.
    LinkDefinition,
}

impl EntityKind {
    /// Returns the earliest normalized contract that can represent this kind.
    ///
    /// Producers and readers share this boundary so merging or record order
    /// cannot silently downgrade a document or legalize an invalid input.
    #[must_use]
    pub const fn minimum_ir_version(self) -> NormalizedIrVersion {
        match self {
            Self::DocumentSection | Self::LinkDefinition => NormalizedIrVersion::V1_5,
            ...
            | Self::Change
            | Self::CommunityView
            | Self::ExternalSymbol => NormalizedIrVersion::V1_1,
        }
    }
}
...
/// A strictly versioned normalized fact document.
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
#[cfg_attr(feature = "schema", derive(schemars::JsonSchema))]
#[serde(deny_unknown_fields)]
pub struct NormalizedIrDocument {
    /// Exact wire version controlling which common entity kinds are admissible.
...
/// A strictly dispatched legacy or normalized IR document.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum IrDocument {
    /// The frozen legacy 1.0 envelope.
    LegacyV1_0(IrDocumentSchema),
    /// The baseline normalized 1.1 fact document.
    NormalizedV1_1(NormalizedIrDocument),
    /// The normalized 1.2 fact document with stylesheet entity kinds.
    ...
    /// The normalized 1.5 fact document with document sections and link definitions.
    NormalizedV1_5(NormalizedIrDocument),
}
...
impl IrDocument {
    ...
    pub const fn version(&self) -> IrVersion {
        match self {
            ...
            Self::NormalizedV1_5(_) => NORMALIZED_IR_VERSION_V1_5,
        }
    }
}
...
pub enum IrDocumentDecodeError {
    ...
    MalformedDocument,
    /// The document declared a version other than exactly 1.0, 1.1, or 1.2.
    ...
    /// A normalized document failed quota or semantic validation.
...
/// envelope. Normalized versions are validated against raw quotas and returned in
...
pub fn decode_ir_document_with_checkpoint(
    ...
    if version != IR_VERSION
        ...
        && version != NORMALIZED_IR_VERSION_V1_5
    {
    ...
    match version {
        ...
        NORMALIZED_IR_VERSION_V1_2 => {
            decode_current_normalized_document(encoded, limits, extensions, &mut checkpoint)
                .map(IrDocument::NormalizedV1_2)
        }
        ...
        NORMALIZED_IR_VERSION_V1_5 => {
            decode_current_normalized_document(encoded, limits, extensions, &mut checkpoint)
                .map(IrDocument::NormalizedV1_5)
        }
...
mod tests {
    ...
    #[test]
    fn stylesheet_kinds_require_their_explicit_document_version() {
        for kind in ["style_rule", "keyframes"] {
            let mut value = normalized_value();
            value["entities"][0]["kind"] = serde_json::json!(kind);
            assert!(decode(&encode_test_value(&value), &IrLimits::default()).is_err());

            value["version"]["minor"] = serde_json::json!(2);
            let decoded = decode(&encode_test_value(&value), &IrLimits::default())
                .expect("stylesheet kinds are supported by explicit IR 1.2");
            assert_eq!(decoded.version(), IrVersion::new(1, 2));
        }
    }

    #[test]
    fn stylesheet_versions_round_trip_all_bounded_decoders_and_reject_downgrades() {
        for kind in ["style_rule", "keyframes"] {
            let mut value = normalized_value();
            value["version"]["minor"] = serde_json::json!(2);
            value["entities"][0]["kind"] = serde_json::json!(kind);
            ...
            let IrDocument::NormalizedV1_2(mut document) =
                decode(&encoded, &limits).expect("versioned fixture decodes")
            else {
                panic!("expected IR 1.2");
            };
            assert_eq!(
                ...
                .expect("direct JSON decoder accepts IR 1.2"),
                document,
            );
            let msgpack = rmp_serde::to_vec_named(&document).expect("MessagePack encodes");
            assert_eq!(
                ...
                .expect("MessagePack decoder accepts IR 1.2"),
                document
            );
            assert_eq!(
            ...
            assert!(serde_json::to_vec(&IrDocument::NormalizedV1_1(document.clone())).is_err());
            assert_eq!(
                serde_json::to_value(IrDocument::NormalizedV1_2(document.clone()))
                    .expect("matching wrapper serializes")["version"]["minor"],
                2
            );

            document.version = NormalizedIrVersion::V1_1;
            assert!(crate::validate_ir_document(&document, &limits, &extensions).is_err());
            let downgraded = serde_json::to_vec(&document).expect("unvalidated value serializes");
            assert!(
                decode_normalized_ir_document_with_checkpoint(
                    &downgraded,
                    &limits,
            ...
            let downgraded =
                rmp_serde::to_vec_named(&document).expect("unvalidated MessagePack encodes");
            assert!(
                decode_normalized_ir_document_messagepack_reader_with_checkpoint(
        ...
        assert_eq!(
            crate::entity_kind_identity_label(EntityKind::StyleRule),
            "style-rule"
        );
        assert_eq!(
            crate::entity_kind_identity_label(EntityKind::Keyframes),
            "keyframes"
        );
    }

    #[test]
    fn markup_versions_round_trip_all_bounded_decoders_and_reject_downgrades() {
        for kind in ["markup_element", "markup_attribute"] {
            let mut value = normalized_value();
            value["version"]["minor"] = serde_json::json!(3);
            value["entities"][0]["kind"] = serde_json::json!(kind);
            let encoded = encode_test_value(&value);
            let limits = IrLimits::default();
            let extensions = ExtensionSupport::default();
            let IrDocument::NormalizedV1_3(mut document) =
                decode(&encoded, &limits).expect("versioned fixture decodes")
            else {
                panic!("expected IR 1.3");
            };
            assert_eq!(
                decode_normalized_ir_document_with_checkpoint(
                    &encoded,
                ...
                .expect("direct JSON decoder accepts IR 1.3"),
                document,
            );
            let msgpack = rmp_serde::to_vec_named(&document).expect("MessagePack encodes");
            assert_eq!(
                ...
                .expect("MessagePack decoder accepts IR 1.3"),
                document
            );
            assert_eq!(
                decode_ir_document_with_checkpoint(&encoded, &limits, &extensions, || false),
            ...
            assert!(serde_json::to_vec(&IrDocument::NormalizedV1_1(document.clone())).is_err());
            assert_eq!(
                serde_json::to_value(IrDocument::NormalizedV1_3(document.clone()))
                    .expect("matching wrapper serializes")["version"]["minor"],
                3
            );
        ...
        assert_eq!(
            crate::entity_kind_identity_label(EntityKind::MarkupElement),
            "markup-element"
        );
        assert_eq!(
            crate::entity_kind_identity_label(EntityKind::MarkupAttribute),
            "markup-attribute"
        );
    }

    #[test]
    fn markup_kinds_reject_both_older_contracts() {
        for kind in ["markup_element", "markup_attribute"] {
            for minor in [1, 2] {
                let mut value = normalized_value();
                value["entities"][0]["kind"] = serde_json::json!(kind);
    ...
    #[test]
    fn versioned_kinds_round_trip_bounded_decoders_and_reject_every_downgrade() {
        let limits = IrLimits::default();
        let extensions = ExtensionSupport::default();
        for (kind, wire, identity) in [
            ...
            assert_eq!(kind.minimum_ir_version(), version);
            assert_eq!(crate::entity_kind_identity_label(kind), identity);
            let mut value = normalized_value();
            value["version"]["minor"] = serde_json::json!(version.value().minor());
            value["entities"][0]["kind"] = serde_json::json!(wire);
            let encoded = encode_test_value(&value);
            let dispatched = decode(&encoded, &limits).unwrap();
            assert_eq!(dispatched.version(), version.value());
            let document = match &dispatched {
                IrDocument::NormalizedV1_4(document) | IrDocument::NormalizedV1_5(document) => {
                    document.clone()
                }
                _ => panic!("expected the exact required normalized version"),
            };
            ...
            for wrapper in [
                ...
                IrDocument::NormalizedV1_4(document.clone()),
            ] {
                if wrapper.version() < version.value() {
    ...
    fn bounded_dispatch_rejects_malformed_and_unsupported_versions() {
        ...
        let mut value = normalized_value();
        value["version"]["minor"] = serde_json::json!(6);
        assert_eq!(
            decode(&encode_test_value(&value), &IrLimits::default()),
            Err(IrDocumentDecodeError::UnsupportedVersion { major: 1, minor: 6 })
        );
    }

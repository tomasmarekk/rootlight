...
use rootlight_mcp_contract::{
    ...
    source_entity::{
        ChangeImpactOutputV1_3, ChangeImpactOutputV1_4, CodeLocateOutputV1_1, CodeLocateOutputV1_2,
        CodeLocateOutputV1_3, HistoryCompareOutputV1_3, HistoryCompareOutputV1_4,
        QueryAdvancedOutputV1_1, QueryAdvancedOutputV1_2, QueryAdvancedOutputV1_3,
...
const STORAGE_COMPATIBILITY_BASELINES: [&str; 7] = [
    ...
    "storage/1.2/oracle-7-schema-fingerprints.json",
];
...
const COMPATIBILITY_BASELINES: [&str; 13] = [
    ...
    STORAGE_COMPATIBILITY_BASELINES[6],
];
const DAEMON_PROTOCOL_DESCRIPTOR_BASELINES: [(&str, &str); 15] = [
    ...
    ("1.17", "protobuf/1.17/rootlight.desc"),
];
const SCHEMA_PROVENANCE_INPUTS: [&str; 24] = [
    ...
    "crates/rootlight-mcp-contract/src/source_entity.rs",
...
fn generate_protobuf(workspace_root: &Path, staged_root: &Path) -> Result<(), SchemaError> {
    ...
    rewrite_legacy_lease_rustdoc(&output_root)?;
...
fn rewrite_legacy_lease_rustdoc(output_root: &Path) -> Result<(), SchemaError> {
    let path = output_root.join("rootlight.daemon.v1.rs");
    let bytes = read_bytes(&path)?;
    let source = String::from_utf8(bytes).map_err(|source| SchemaError::GeneratedRustUtf8 {
        path: path.clone(),
        source,
    })?;
    let replacements = [
    ...
    for (expected, replacement) in replacements {
        if !rewritten.contains(expected) {
            return Err(SchemaError::GeneratedRustdocTemplate(expected));
        }
        rewritten = rewritten.replacen(expected, replacement, 1);
    }
...
fn generate_json_schemas(workspace_root: &Path, staged_root: &Path) -> Result<(), SchemaError> {
    ...
    write_normalized_ir_schema(&schema_root.join("ir-1.5.schema.json"))?;
    ...
    write_mcp_tool_schema_version::<CodeLocateInput>(&schema_root, "code.locate", "input", "1.5")?;
    write_mcp_tool_schema_version::<rootlight_mcp_contract::source_entity::CodeLocateOutputV1_5>(
        &schema_root,
        "code.locate",
        "output",
        "1.5",
    ...
    write_mcp_tool_schema_version::<rootlight_mcp_contract::source_entity::QueryAdvancedOutputV1_5>(
        &schema_root,
        "query.advanced",
        "output",
        "1.5",
    ...
    write_mcp_tool_schema_version::<rootlight_mcp_contract::source_entity::SymbolExplainOutputV1_6>(
        &schema_root,
        "symbol.explain",
        "output",
        "1.6",
    ...
    write_mcp_tool_schema_version::<QueryAdvancedOutputV1_3>(
        &schema_root,
        "query.advanced",
        "output",
        "1.3",
    ...
    write_mcp_tool_schema_version::<SymbolExplainOutputV1_4>(
        &schema_root,
        "symbol.explain",
        "output",
        "1.4",
    ...
    write_mcp_tool_schema_version::<ChangeImpactOutputV1_4>(
        &schema_root,
        "change.impact",
        "output",
        "1.4",
    ...
    write_mcp_tool_schema_version::<HistoryCompareOutputV1_4>(
        &schema_root,
        "history.compare",
        "output",
        "1.4",
    ...
    write_mcp_tool_schema_version::<CodeLocateInput>(&schema_root, "code.locate", "input", "1.2")?;
    write_mcp_tool_schema_version::<CodeLocateOutputV1_2>(
        &schema_root,
        "code.locate",
        "output",
        "1.2",
    ...
    write_mcp_tool_schema_version::<SymbolExplainOutputV1_3>(
        &schema_root,
        "symbol.explain",
        "output",
        "1.3",
    ...
    write_mcp_tool_schema_version::<QueryAdvancedOutputV1_2>(
        &schema_root,
        "query.advanced",
        "output",
        "1.2",
    ...
    write_mcp_tool_schema_version::<CodeLocateInput>(&schema_root, "code.locate", "input", "1.1")?;
    write_mcp_tool_schema_version::<CodeLocateOutputV1_1>(
        &schema_root,
        "code.locate",
        "output",
        "1.1",
    ...
    write_mcp_tool_schema_version::<SymbolExplainOutputV1_2>(
        &schema_root,
        "symbol.explain",
        "output",
        "1.2",
    ...
    write_mcp_tool_schema_version::<QueryAdvancedOutputV1_1>(
        &schema_root,
        "query.advanced",
        "output",
        "1.1",
    ...
    write_mcp_tool_schema_version::<ChangeImpactOutputV1_3>(
        &schema_root,
        "change.impact",
        "output",
        "1.3",
    ...
    write_mcp_tool_schema_version::<HistoryCompareOutputV1_3>(
        &schema_root,
        "history.compare",
        "output",
        "1.3",
...
fn write_normalized_ir_schema(path: &Path) -> Result<(), SchemaError> {
    ...
    // schema names exactly one contract. The 1.1 through 1.4 artifacts remain frozen.
    let minor = schema
        .as_object_mut()
    ...
    *minor = serde_json::json!({"type": "integer", "const": 5});
    write_schema_value(path, schema)
}
...
fn write_schema_value(path: &Path, mut schema: schemars::Schema) -> Result<(), SchemaError> {
    // An IR vocabulary extension cannot widen a retained MCP contract. These
    // exact revisions introduce source kinds at the agent-facing boundary.
    if path
        .file_name()
            ...
            .and_then(serde_json::Value::as_array_mut)
    {
        let name = path
            ...
            .and_then(std::ffi::OsStr::to_str)
            .unwrap_or("");
        let binding_kinds = matches!(
            name,
            "mcp-code-locate-input-1.5.schema.json"
                | "mcp-code-locate-output-1.5.schema.json"
                | "mcp-query-advanced-input-1.5.schema.json"
                | "mcp-query-advanced-output-1.5.schema.json"
                | "mcp-symbol-explain-input-1.6.schema.json"
                | "mcp-symbol-explain-output-1.6.schema.json"
        );
        let document_kinds = binding_kinds
            || matches!(
                name,
        ...
        let database_query_kinds = declaration_kinds
            || matches!(
                name,
                "mcp-code-locate-input-1.2.schema.json"
                    | "mcp-code-locate-output-1.2.schema.json"
                    | "mcp-symbol-explain-input-1.3.schema.json"
                    | "mcp-symbol-explain-output-1.3.schema.json"
                    | "mcp-query-advanced-input-1.2.schema.json"
                    | "mcp-query-advanced-output-1.2.schema.json"
            );
        let database_kinds = database_query_kinds
            || name.starts_with("mcp-change-impact-output-")
            || name.starts_with("mcp-history-compare-output-");
        let source_kinds = database_query_kinds
            || matches!(
                name,
                "mcp-code-locate-input-1.1.schema.json"
                    | "mcp-code-locate-output-1.1.schema.json"
                    | "mcp-symbol-explain-input-1.2.schema.json"
                    | "mcp-symbol-explain-output-1.2.schema.json"
                    | "mcp-query-advanced-input-1.1.schema.json"
                    | "mcp-query-advanced-output-1.1.schema.json"
                    | "mcp-change-impact-input-1.3.schema.json"
                    | "mcp-change-impact-output-1.3.schema.json"
                    | "mcp-history-compare-input-1.3.schema.json"
                    | "mcp-history-compare-output-1.3.schema.json"
            );
        let stylesheet_kinds = source_kinds
            || matches!(
                name,
                "mcp-change-impact-output-1.2.schema.json"
                    | "mcp-history-compare-output-1.2.schema.json"
            );
        variants.retain(
            |variant| match variant.get("const").and_then(serde_json::Value::as_str) {
                // Change/history expose the IR enum, which already includes bindings.
                Some("import" | "export") => {
                    binding_kinds
                        || name.starts_with("mcp-change-impact-output-")
                        || name.starts_with("mcp-history-compare-output-")
...
fn validate_generated_json_schemas(
    ...
    let mut markup_ir = stylesheet_ir.clone();
    markup_ir["version"]["minor"] = serde_json::json!(3);
    ...
    let mut cases = vec![
        ...
        SchemaSemanticCase::valid("ir-1.3.schema.json", "markup element document", markup_ir),
        SchemaSemanticCase::valid(
            "ir-1.3.schema.json",
            "markup attribute document",
            attribute_ir,
        ),
        SchemaSemanticCase::invalid(
            "ir-1.2.schema.json",
            "markup kind in older vocabulary",
            downgraded_markup_ir.clone(),
        ),
        SchemaSemanticCase::invalid(
            "ir-1.3.schema.json",
            "markup contract version downgrade",
            downgraded_markup_ir,
        ),
        SchemaSemanticCase::valid(
            "ir-1.2.schema.json",
    ...
    for kind in ["document_section", "link_definition"] {
        let mut value = declaration_ir.clone();
        value["version"]["minor"] = serde_json::json!(5);
        value["entities"][0]["kind"] = serde_json::json!(kind);
        cases.push(SchemaSemanticCase::valid(
            "ir-1.5.schema.json",
            "authored document entity kind",
            value.clone(),
        ));
        for (minor, schema) in [
            (1, "ir-1.1.schema.json"),
            (2, "ir-1.2.schema.json"),
            (3, "ir-1.3.schema.json"),
            (4, "ir-1.4.schema.json"),
        ] {
            value["version"]["minor"] = serde_json::json!(minor);
            cases.push(SchemaSemanticCase::invalid(
                schema,
                "document entity in older vocabulary",
                value.clone(),
            ));
            cases.push(SchemaSemanticCase::invalid(
                "ir-1.5.schema.json",
                "document version downgrade",
                value.clone(),
            ));
        }
    }
            ...
            match case.schema {
                ...
                "ir-1.5.schema.json" => {
                    ...
                    matches!(
                        ...
                        Ok(IrDocument::NormalizedV1_5(_))
                    )
                }
...
fn expected_artifact_paths() -> Vec<String> {
    ...
    paths.extend([
        ...
        format!("{SCHEMA_ROOT}/json/ir-1.5.schema.json"),
        ...
        format!("{SCHEMA_ROOT}/json/mcp-code-locate-input-1.5.schema.json"),
        format!("{SCHEMA_ROOT}/json/mcp-code-locate-output-1.5.schema.json"),
        format!("{SCHEMA_ROOT}/json/mcp-query-advanced-input-1.5.schema.json"),
        format!("{SCHEMA_ROOT}/json/mcp-query-advanced-output-1.5.schema.json"),
        format!("{SCHEMA_ROOT}/json/mcp-symbol-explain-input-1.6.schema.json"),
        format!("{SCHEMA_ROOT}/json/mcp-symbol-explain-output-1.6.schema.json"),
        ...
        format!("{SCHEMA_ROOT}/json/mcp-query-advanced-output-1.3.schema.json"),
        format!("{SCHEMA_ROOT}/json/mcp-symbol-explain-input-1.4.schema.json"),
        format!("{SCHEMA_ROOT}/json/mcp-symbol-explain-output-1.4.schema.json"),
        format!("{SCHEMA_ROOT}/json/mcp-change-impact-input-1.4.schema.json"),
        format!("{SCHEMA_ROOT}/json/mcp-change-impact-output-1.4.schema.json"),
        format!("{SCHEMA_ROOT}/json/mcp-history-compare-input-1.4.schema.json"),
        format!("{SCHEMA_ROOT}/json/mcp-history-compare-output-1.4.schema.json"),
        format!("{SCHEMA_ROOT}/json/mcp-code-locate-input-1.2.schema.json"),
        format!("{SCHEMA_ROOT}/json/mcp-code-locate-output-1.2.schema.json"),
        format!("{SCHEMA_ROOT}/json/mcp-symbol-explain-input-1.3.schema.json"),
        format!("{SCHEMA_ROOT}/json/mcp-symbol-explain-output-1.3.schema.json"),
        format!("{SCHEMA_ROOT}/json/mcp-query-advanced-input-1.2.schema.json"),
        format!("{SCHEMA_ROOT}/json/mcp-query-advanced-output-1.2.schema.json"),
        format!("{SCHEMA_ROOT}/json/mcp-code-locate-output-1.0.schema.json"),
        format!("{SCHEMA_ROOT}/json/mcp-code-locate-input-1.1.schema.json"),
        format!("{SCHEMA_ROOT}/json/mcp-code-locate-output-1.1.schema.json"),
        format!("{SCHEMA_ROOT}/json/mcp-symbol-explain-input-1.2.schema.json"),
        format!("{SCHEMA_ROOT}/json/mcp-symbol-explain-output-1.2.schema.json"),
        format!("{SCHEMA_ROOT}/json/mcp-query-advanced-input-1.1.schema.json"),
        format!("{SCHEMA_ROOT}/json/mcp-query-advanced-output-1.1.schema.json"),
        format!("{SCHEMA_ROOT}/json/mcp-change-impact-input-1.3.schema.json"),
        format!("{SCHEMA_ROOT}/json/mcp-change-impact-output-1.3.schema.json"),
        format!("{SCHEMA_ROOT}/json/mcp-history-compare-input-1.3.schema.json"),
        format!("{SCHEMA_ROOT}/json/mcp-history-compare-output-1.3.schema.json"),
...
pub(crate) enum SchemaError {
    ...
    #[error("generated Rust artifact is not UTF-8 at {path}")]
    GeneratedRustUtf8 {
        path: PathBuf,
        #[source]
        source: std::string::FromUtf8Error,
    },
    #[error("generated daemon Rustdoc template is missing: {0}")]
    GeneratedRustdocTemplate(&'static str),

...
impl SessionRegistry {
    ...
    pub(crate) fn consume_bootstrap(
        ...
        reap_expired(&mut state, now);
        let index = constant_time_record_index(&state.bootstraps, &digest)?;
        state.bootstraps.remove(index);
        if state.sessions.len() >= MAX_SESSIONS {
            return None;
        }
        let session_id = random_secret().ok()?;
        let csrf = random_secret().ok()?;
...
fn constant_time_record_index(records: &[BootstrapRecord], candidate: &[u8; 32]) -> Option<usize> {
    records
        .iter()
        .enumerate()
...
mod tests {
    ...
    #[test]
    fn bootstrap_is_single_use_and_sessions_require_exact_csrf() {
        ...
        let credentials = registry
            ...
            .expect("bootstrap creates a session");
        assert!(
            registry
                .consume_bootstrap(bootstrap.encoded(), now)
                .is_none()
    ...
    #[test]
    fn bootstrap_and_idle_expiry_fail_closed() {
        let registry = SessionRegistry::new();
        let now = Instant::now();
        let bootstrap = registry
            .issue_bootstrap(now)
            .expect("bootstrap secret issues");
        let after_bootstrap_expiry = now
            .checked_add(BOOTSTRAP_TTL + Duration::from_nanos(1))
            .expect("test time is representable");
        assert!(
            registry
                .consume_bootstrap(bootstrap.encoded(), after_bootstrap_expiry)
                .is_none()
        );

        let bootstrap = registry
            .issue_bootstrap(now)
            .expect("second bootstrap secret issues");
        let credentials = registry
            .consume_bootstrap(bootstrap.encoded(), now)
            .expect("bootstrap creates a session");
        ...
        let bootstrap = registry
            .issue_bootstrap(now)
            .expect("third bootstrap secret issues");
        let credentials = registry
            .consume_bootstrap(bootstrap.encoded(), now)
            .expect("bootstrap creates another session");

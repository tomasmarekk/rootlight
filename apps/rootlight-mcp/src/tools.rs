...
use rootlight_mcp_contract::{
    ...
    source_entity::{
        ChangeImpactOutputV1_3, ChangeImpactOutputV1_4, ChangeImpactOutputV1_5,
        CodeLocateOutputV1_1, CodeLocateOutputV1_2, CodeLocateOutputV1_3, CodeLocateOutputV1_4,
        CodeLocateOutputV1_5, HistoryCompareOutputV1_3, HistoryCompareOutputV1_4,
        HistoryCompareOutputV1_5, QueryAdvancedOutputV1_1, QueryAdvancedOutputV1_2,
        QueryAdvancedOutputV1_3, QueryAdvancedOutputV1_4, QueryAdvancedOutputV1_5,
        SymbolExplainOutputV1_2, SymbolExplainOutputV1_3, SymbolExplainOutputV1_4,
        SymbolExplainOutputV1_5, SymbolExplainOutputV1_6,
    },
    vertical::{
...
impl ContractSelection {
    fn select(contract: &ToolContract, requested: Option<&str>) -> Option<Self> {
        match requested {
            ...
            Some(requested)
                if contract.tool.third_legacy_contract_version() == Some(requested)
                    && contract.third_legacy_output_validator.is_some() =>
            {
...
impl ToolContract {
    fn compile(tool: VerticalTool) -> Result<Self, ToolRegistryError> {
        ...
        let second_legacy_input_validator =
            tool.second_legacy_input_schema_json()
                ...
                .transpose()?;
        let third_legacy_input_validator =
            tool.third_legacy_input_schema_json()
                .map(|schema| {
                    let schema = parse_object_schema(tool, "third legacy input", schema).map_err(
                        |source| ToolRegistryError::ParseSchema {
                            tool,
                            direction: "third legacy input",
                            source,
                        },
                    )?;
                    jsonschema::draft202012::new(&Value::Object(schema)).map_err(|source| {
        ...
        let initial_input_validator = tool
            ...
            .map(|schema| {
                let schema =
                    parse_object_schema(tool, "initial input", schema).map_err(|source| {
                        ToolRegistryError::ParseSchema {
                            tool,
                            direction: "initial input",
                            source,
                        }
                    })?;
                jsonschema::draft202012::new(&Value::Object(schema)).map_err(|source| {
        ...
        let second_legacy_output_validator =
            tool.second_legacy_output_schema_json()
                ...
                .transpose()?;
        let third_legacy_output_validator =
            tool.third_legacy_output_schema_json()
                .map(|schema| {
                    let schema = parse_object_schema(tool, "third legacy output", schema).map_err(
                        |source| ToolRegistryError::ParseSchema {
                            tool,
                            direction: "third legacy output",
                            source,
                        },
                    )?;
                    jsonschema::draft202012::new(&Value::Object(schema)).map_err(|source| {
        ...
        let initial_output_validator = tool
            ...
            .map(|schema| {
                let schema =
                    parse_object_schema(tool, "initial output", schema).map_err(|source| {
                        ToolRegistryError::ParseSchema {
                            tool,
                            direction: "initial output",
                            source,
                        }
                    })?;
                jsonschema::draft202012::new(&Value::Object(schema)).map_err(|source| {
    ...
    fn output_validator(&self, selection: ContractSelection) -> &Validator {
        match selection {
            ...
            ContractSelection::ThirdLegacy => self
                .third_legacy_output_validator
                .as_ref()
            ...
            ContractSelection::Initial => self
                .initial_output_validator
                .as_ref()
                .expect("initial selection requires a compiled schema"),
        }
    }
...
fn typed_output_is_valid(tool: VerticalTool, input: &TypedInput, output: &Value) -> bool {
    ...
    match tool {
        ...
        VerticalTool::QueryAdvanced => QueryAdvancedOutputV1_5::deserialize(output).is_ok(),
        VerticalTool::OperationStatus => OperationStatusOutput::deserialize(output).is_ok(),
        VerticalTool::CodeLocate => CodeLocateOutputV1_5::deserialize(output).is_ok(),
        VerticalTool::SymbolExplain => SymbolExplainOutputV1_6::deserialize(output).is_ok(),
        VerticalTool::SymbolRelationships => {
            SymbolRelationshipsOutputV1_1::deserialize(output).is_ok()
        }
        VerticalTool::ChangeImpact => ChangeImpactOutputV1_5::deserialize(output).is_ok(),
        ...
        VerticalTool::HistoryCompare => HistoryCompareOutputV1_5::deserialize(output).is_ok(),
...
fn typed_selected_output_is_valid(
    ...
    match selection {
        ...
        ContractSelection::Previous => match tool {
            VerticalTool::CodeLocate => CodeLocateOutputV1_4::deserialize(output).is_ok(),
            VerticalTool::QueryAdvanced => QueryAdvancedOutputV1_4::deserialize(output).is_ok(),
            VerticalTool::SymbolExplain => SymbolExplainOutputV1_5::deserialize(output).is_ok(),
            VerticalTool::ChangeImpact => ChangeImpactOutputV1_4::deserialize(output).is_ok(),
            VerticalTool::HistoryCompare => HistoryCompareOutputV1_4::deserialize(output).is_ok(),
        ...
        ContractSelection::Legacy => match tool {
            VerticalTool::CodeLocate => CodeLocateOutputV1_3::deserialize(output).is_ok(),
            VerticalTool::QueryAdvanced => QueryAdvancedOutputV1_3::deserialize(output).is_ok(),
            VerticalTool::SymbolExplain => SymbolExplainOutputV1_4::deserialize(output).is_ok(),
        ...
        ContractSelection::SecondLegacy => match tool {
            VerticalTool::CodeLocate => CodeLocateOutputV1_2::deserialize(output).is_ok(),
            VerticalTool::QueryAdvanced => QueryAdvancedOutputV1_2::deserialize(output).is_ok(),
            VerticalTool::SymbolExplain => SymbolExplainOutputV1_3::deserialize(output).is_ok(),
...
fn tool_error(
    ...
    let mut error = serde_json::to_value(error).map_err(|_| ToolResultError::Serialize)?;
    select_public_error_version(contract.tool, selection, &mut error)
...
fn requires_entity_contract(
    ...
    let is_new_kind = |value: Option<&Value>| match value.and_then(Value::as_str) {
        Some("import" | "export") => match tool {
            VerticalTool::CodeLocate
            | VerticalTool::QueryAdvanced
            | VerticalTool::SymbolExplain => selection != ContractSelection::Current,
            _ => false,
        },
        Some("document_section" | "link_definition") => match tool {
            VerticalTool::CodeLocate | VerticalTool::QueryAdvanced => {
                matches!(
                    selection.schema_version(tool),
                    "1.0" | "1.1" | "1.2" | "1.3"
                )
            }
            VerticalTool::SymbolExplain
        ...
        Some("event" | "error_declaration" | "modifier") => match tool {
            VerticalTool::CodeLocate | VerticalTool::QueryAdvanced => {
                matches!(selection.schema_version(tool), "1.0" | "1.1" | "1.2")
            }
            VerticalTool::SymbolExplain
            | VerticalTool::ChangeImpact
            | VerticalTool::HistoryCompare => {
        ...
        Some("markup_element" | "markup_attribute") => match tool {
            VerticalTool::CodeLocate | VerticalTool::QueryAdvanced => {
                selection.schema_version(tool) == "1.0"
            }
            VerticalTool::SymbolExplain => matches!(selection.schema_version(tool), "1.0" | "1.1"),
            VerticalTool::ChangeImpact | VerticalTool::HistoryCompare => {
                matches!(selection.schema_version(tool), "1.0" | "1.1" | "1.2")
            }
            _ => false,
        },
        Some("style_rule" | "keyframes") => match tool {
            VerticalTool::CodeLocate | VerticalTool::QueryAdvanced => {
                selection.schema_version(tool) == "1.0"
            }
            _ => matches!(selection.schema_version(tool), "1.0" | "1.1"),
        },
        _ => false,
    };
    let items = |pointer| {
...
fn select_output_version(
    ...
    let data = root
        ...
        .ok_or(())?;
    if tool == VerticalTool::OperationStatus {
        select_public_error_version(tool, selection, data.get_mut("error").ok_or(())?)?;
    }
    match tool {
        ...
        VerticalTool::SymbolExplain => {
            if selection.schema_version(tool) != "1.0" {
                return Ok(output);
            }
            let symbols = data
                .get_mut("symbols")
...
fn remove_operation_fact_work(data: &mut Map<String, Value>) -> Result<(), ()> {
    let Some(incremental) = data.get_mut("incremental") else {
        return Ok(());
    };
    if incremental.is_null() {
        return Ok(());
    }
    incremental.as_object_mut().ok_or(())?.remove("fact_work");
    Ok(())
}
...
fn select_public_error_version(
    tool: VerticalTool,
    selection: ContractSelection,
    error: &mut Value,
) -> Result<(), ()> {
    // Only repository operations expose configuration/catalog mutations in
    // their error vocabulary; other current tools retain the baseline hints.
    let retains_extended_actions = matches!(
        (tool, selection),
        (
            VerticalTool::RepoIndex,
    ...
    let actions = error
        .as_object_mut()
        .and_then(|error| error.get_mut("next_actions"))
        .and_then(Value::as_array_mut)
        .ok_or(())?;
    actions.retain(|action| {
        !matches!(
            action.get("action").and_then(Value::as_str),
            Some("update_configuration" | "delete_repository")
        )
    });
    Ok(())
}
...
mod tests {
    ...
    async fn generated_restricted_rules_reject_direct_calls_without_execution() {
        ...
        assert_eq!((declared, covered, exclusions.len()), (48, 48, 0));
    }
    ...
    async fn tools_list_payloads_match_profile_goldens() {
        ...
        assert_eq!(
            ...
            [
                (
                    231_313,
                    "c35b9d3071eabf1a472543ce32086ffddf024472b485959960e3b21909728e9b".to_owned(),
                ),
                (
                    500_910,
                    "c0ff9b47af83b2c048bcbdbfc03faa3e1461dcfca283bcfa96c4e8460ded0075".to_owned(),
                ),
                (
                    704_269,
                    "3c4eb5d388418007f4b2b089fa51a676ef0403b3c667149f8529ea952e287995".to_owned(),
                ),
            ],
        ...
        for (tool, field) in [
            ...
            for kind in [
                ...
                for (version, schema) in [
                    ("1.5", tool.output_schema_json()),
                    ("1.4", tool.previous_output_schema_json().unwrap()),
                    ("1.3", tool.legacy_output_schema_json().unwrap()),
                    ("1.2", tool.second_legacy_output_schema_json().unwrap()),
                    ("1.1", tool.third_legacy_output_schema_json().unwrap()),
                    ("1.0", tool.fourth_legacy_output_schema_json().unwrap()),
                ] {
                    let mut expected = output.clone();
                    expected["schema_version"] = json!(version);
                    let minimum = match kind {
                        "document_section" | "link_definition" => 5,
                        "event" | "error_declaration" | "modifier" => 4,
                        "markup_element" | "markup_attribute" => 3,
                        "style_rule" | "keyframes" => 2,
                        _ => 0,
                    };
    ...
    async fn malformed_change_entities_remain_internal_errors_for_retained_clients() {
        for (tool, field) in [
            ...
            for (version, kind) in [
                ...
                ("1.1", "style_rule"),
                ("1.2", "style_rule"),
                ("1.0", "document_section"),
                ("1.1", "document_section"),
                ("1.2", "document_section"),
                ("1.3", "document_section"),
                ("1.4", "document_section"),
            ] {
                let mut output = change_entity_output(tool, field, kind);
    ...
    fn change_entity_version_detection_ignores_non_entity_text() {
        for tool in [VerticalTool::ChangeImpact, VerticalTool::HistoryCompare] {
            let mut output = Value::Object(retained_output(tool.name()));
            for (display, trace) in [
                ("style_rule", "keyframes"),
                ("document_section", "link_definition"),
            ] {
                output["repository"]["display_name"] = json!(display);
                output["usage"]["trace_id"] = json!(trace);
    ...
    async fn repo_index_serves_current_and_three_retained_contracts() {
        ...
        for (version, semantic_operation) in [("1.2", true), ("1.1", true), ("1.0", false)] {
            ...
            assert_eq!(
                retained["structuredContent"]["data"]
    ...
    #[tokio::test]
    async fn declaration_contracts_preserve_supported_remediation_actions() {
        ...
        .build()
        .expect("bounded error builds");
        let router = ToolRouter::new(
            StaticExecutor {
                result: Err(ToolExecutionError::new(error)),
            },
            ExposureProfile::Developer,
        )
        .expect("registry compiles");
        for tool in [
            ...
            .flatten()
            {
                ...
                let structured = &result["structuredContent"];
                assert_eq!(result["isError"], true);
                assert_eq!(structured["schema_version"], version);
                assert_eq!(structured["error"]["code"], "RESOURCE_EXHAUSTED");
                assert_eq!(
                    structured["error"]["next_actions"],
                    json!([{"action": "collect_support_bundle"}]),
                    "{} {version} retains its remediation vocabulary",
                    tool.name()
                );
            }
        }
    ...
    #[test]
    fn remediation_projection_matches_every_current_and_retained_schema() {
        for tool in VerticalTool::ALL {
            for (selection, schema) in [
                (ContractSelection::Current, Some(tool.output_schema_json())),
                (
                    ContractSelection::Previous,
                    tool.previous_output_schema_json(),
                ),
                (ContractSelection::Legacy, tool.legacy_output_schema_json()),
                (
                    ContractSelection::SecondLegacy,
                    tool.second_legacy_output_schema_json(),
                ),
                ...
                let mut error = json!({"next_actions": [
                    {"action": "collect_support_bundle"},
                    {"action": "update_configuration", "key": "analysis.max_repositories"},
                    {"action": "delete_repository"}
                ]});
                select_public_error_version(tool, selection, &mut error)
                    .expect("valid error projects");
                let extended = schema.contains("\"delete_repository\"");
                assert_eq!(extended, schema.contains("\"update_configuration\""));
                assert_eq!(
                    error["next_actions"][0],
                    json!({"action": "collect_support_bundle"})
                );
                assert_eq!(
                    error["next_actions"].as_array().expect("actions").len(),
                    if extended { 3 } else { 1 },
                    "{} {} follows its frozen remediation vocabulary",
                    tool.name(),
                    selection.schema_version(tool)
                );
            }
    ...
    #[tokio::test]
    async fn repo_index_retained_errors_strip_current_remediation_actions() {
        let error = PublicError::builder(
            ErrorCode::ResourceExhausted,
            "repository capacity is exhausted",
        )
        .next_action(NextAction::UpdateConfiguration {
            key: SafeLabel::parse("analysis.max_repositories")
                .expect("hard-coded configuration key is valid"),
        })
        .next_action(NextAction::DeleteRepository)
        .build()
        .expect("bounded error builds");
    ...
    async fn operation_status_serves_current_and_six_retained_minor_contracts() {
        let Value::Object(output) = json!({
            ...
            "data": {
                ...
                "planning": {
                    ...
                    "estimated_durable_bytes": 1024,
                    "dependency_keys": {
                        "total": 1,
                        "samples": [{
                            "kind": "file_content",
                            "subject": rootlight_ids::FileId::from_bytes([3; 20])
                        }],
                        "complete": true
                    }
                },
                    ...
                    "fact_work": {
                        "planned": {
                            "groups": [{
                                "disposition": "reuse",
                                "domain": "syntax",
                                "provider_pass": "parser",
                                "cause": "complete_dependency_match",
                                "affected_files": {
                                    "total": 1,
                                    "samples": [rootlight_ids::FileId::from_bytes([3; 20])],
                                    "complete": true
                                },
                                "affected_analysis_units": {
                                    "total": 1,
                                    "samples": [rootlight_ids::FactId::from_bytes([4; 20])],
                                    "complete": true
                                }
                            }],
                            "total_groups": 1,
                            ...
                            "groups": [
                                {
                                    ...
                                    "domain": "entities",
                                    "provider_pass": "lower",
                                    "cause": "generation_bound_lowering",
                                    "fact_count": 4,
                                    "affected_files": {
                                        "total": 1,
                                        "samples": [rootlight_ids::FileId::from_bytes([3; 20])],
        ...
        for (
            ...
            assert_eq!(
                data.get("incremental")
                    .and_then(|incremental| incremental.get("fact_work"))
                    .is_some(),
                fact_work
            );
            assert_eq!(
                data["error"]["next_actions"].as_array().map_or(0, Vec::len),
                actions
            );
        }
    }
    ...
    #[tokio::test]
    async fn clean_rebuild_status_is_retained_in_one_four_and_stripped_from_older_versions() {
        let Value::Object(output) = json!({
            "schema_version": "1.5",
            "data": {
                "operation": {
        ...
        for version in ["1.5", "1.4", "1.3", "1.2", "1.1", "1.0"] {
            let result = success(
                router
                    .handle(
                        request(
    ...
    fn source_kind_scan_admission_is_versioned() {
        ...
        for (minor, schema) in [
            (0, tool.fourth_legacy_input_schema_json().unwrap()),
            (1, tool.third_legacy_input_schema_json().unwrap()),
            (2, tool.second_legacy_input_schema_json().unwrap()),
            (3, tool.legacy_input_schema_json().unwrap()),
            (4, tool.previous_input_schema_json().unwrap()),
            (5, tool.input_schema_json()),
        ] {
            let schema: Value = serde_json::from_str(schema).expect("valid schema");
            let validator = jsonschema::draft202012::new(&schema).expect("schema compiles");
            for (kind, minimum) in [

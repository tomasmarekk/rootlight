...
def _samples(value: Any, field: str, expected: int) -> list[int]:
    if not isinstance(value, list) or len(value) != expected:
        raise LifecycleValidationError(
            f"{field} must contain exactly {expected} samples"
        )
    return [
...
def _validate_health(document: Any, target: str) -> None:
    ...
    if limit != 3_000_000 or any(sample > limit for sample in samples):
        raise LifecycleValidationError(
            "Windows cold-health samples exceed the 3 second limit"
        )
    for field in (
        "successful_attempts",
        "launcher_exit_count",
...
def _validate_mcp(document: Any, target: str) -> None:
    ...
    for field, expected in expected_percentiles.items():
        if _integer(mcp[field], field, minimum=1) != expected:
            raise LifecycleValidationError(
                f"{field} was not recomputed from raw samples"
            )
    limits = {
    ...
    if (
        ...
        raise LifecycleValidationError(
            "installed MCP latency exceeds a release percentile"
        )
    expected_counts = {
    ...
    for field, expected in expected_counts.items():
        if _integer(mcp[field], field) != expected:
            raise LifecycleValidationError(
                f"{field} differs from raw execution evidence"
            )
    ...
    if target == "x86_64-pc-windows-msvc":
        ...
        if _integer(cleanup, "MCP post-cleanup process count") != 0:
            raise LifecycleValidationError(
                "installed MCP cleanup retained a Windows process"
            )
...
def validate_document(
    ...
    *,
    ...
    if (
        _integer(lifecycle["bootstrap_owned_files"], "bootstrap_owned_files", minimum=1)
        < 1
    ):
        raise LifecycleValidationError("package bootstrap owned no files")
    if (
        _integer(lifecycle["uninstall_removed_versions"], "uninstall_removed_versions")
        != 2
    ):
...
def validate_path(
    ...
    *,
    ...
    if not path.is_file():
        raise LifecycleValidationError(
            f"lifecycle evidence is not a regular file: {path}"
        )
    ...
    try:
        ...
        raise LifecycleValidationError(
            "lifecycle evidence is not valid bounded JSON"
        ) from error
    validate_document(
        document,
        target=target,

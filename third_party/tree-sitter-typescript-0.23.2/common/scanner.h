...
static bool is_ecmascript_whitespace(int32_t c) {
    return c == '\t' || c == '\v' || c == '\f' || c == ' ' || c == '\r' || c == '\n' ||
...
static bool scan_whitespace_and_comments(TSLexer *lexer, bool *scanned_comment) {
    for (;;) {
        while (is_ecmascript_whitespace(lexer->lookahead)) {
            skip(lexer);
        }
...
static bool scan_automatic_semicolon(TSLexer *lexer, const bool *valid_symbols, bool *scanned_comment) {
    ...
    for (;;) {
        ...
        if (!is_ecmascript_whitespace(lexer->lookahead)) {
            return false;
        }
        if (lexer->lookahead == '\n') {
            break;
    ...
    switch (lexer->lookahead) {
        case 'f':
            if (valid_symbols[MODULE_FROM_CONTEXT]) {
                const char *word = "from";
                ...
                bool identifier_continues = (c >= 'a' && c <= 'z') ||
                    (c >= 'A' && c <= 'Z') || (c >= '0' && c <= '9') ||
                    c == '_' || c == '$' || c == '\\' ||
                    (c >= 0x80 && !is_ecmascript_whitespace(c));
                // This is lookahead only: ordinary lexing retains comments and
                // the original `from` token instead of hiding them in a scanner token.
                return identifier_continues;
            }
            break;
        case '`':
        case ',':

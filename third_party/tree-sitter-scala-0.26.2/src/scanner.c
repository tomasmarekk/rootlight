...
typedef struct {
  Array(int64_t) indents;
  int64_t last_indentation_size;
  int64_t last_newline_count;
  int64_t last_column;
...
// Positions retain the lexer's complete uint32 domain. The case-region flag
// is separate on the wire, and no successful push can exceed the fixed ABI.
enum {
  STATE_HEADER_SIZE = 18,
  STATE_INDENT_SIZE = 5,
  MAX_INDENTS = (TREE_SITTER_SERIALIZATION_BUFFER_SIZE - STATE_HEADER_SIZE) / STATE_INDENT_SIZE,
};

static void write_u32(char *buffer, uint32_t value) {
  for (unsigned i = 0; i < 4; i++) buffer[i] = (char)(value >> (8 * i));
}
...
unsigned tree_sitter_scala_external_scanner_serialize(void *payload, char *buffer) {
  Scanner *scanner = payload;
  if (scanner->indents.size > MAX_INDENTS) return 0;
  buffer[0] = 1;
  buffer[1] = (char)((scanner->last_indentation_size >= 0) |
                    ((scanner->last_column >= 0) << 1) |
                    (scanner->after_colon_eol << 2));
  write_u32(buffer + 2, scanner->last_indentation_size < 0 ? 0 : (uint32_t)scanner->last_indentation_size);
  write_u32(buffer + 6, (uint32_t)scanner->last_newline_count);
  write_u32(buffer + 10, scanner->last_column < 0 ? 0 : (uint32_t)scanner->last_column);
  write_u32(buffer + 14, (uint32_t)scanner->last_char);
  unsigned size = STATE_HEADER_SIZE;
  for (unsigned i = 0; i < scanner->indents.size; i++) {
    int64_t entry = scanner->indents.contents[i];
    write_u32(buffer + size, (uint32_t)entry);
    buffer[size + 4] = (char)((entry >> 32) & 1);
    size += STATE_INDENT_SIZE;
  }
  return size;
}
...
void tree_sitter_scala_external_scanner_deserialize(void *payload, const char *buffer,
  ...
  // Validate every byte domain before changing the reset state or allocating.
  // Old process-local snapshots have no version marker and are not compatible.
  if (!buffer || length < STATE_HEADER_SIZE ||
      length > TREE_SITTER_SERIALIZATION_BUFFER_SIZE ||
      (length - STATE_HEADER_SIZE) % STATE_INDENT_SIZE != 0 ||
      buffer[0] != 1 || ((uint8_t)buffer[1] & ~7u) != 0) return;
  uint8_t flags = (uint8_t)buffer[1];
  uint32_t indentation = read_u32(buffer + 2);
  uint32_t newlines = read_u32(buffer + 6);
  uint32_t column = read_u32(buffer + 10);
  uint32_t character = read_u32(buffer + 14);
  if (newlines > 2 || character > 0x10FFFF ||
      (character >= 0xD800 && character <= 0xDFFF) ||
      (!(flags & 1) && indentation != 0) ||
      (!(flags & 2) && column != 0)) return;
  unsigned count = (length - STATE_HEADER_SIZE) / STATE_INDENT_SIZE;
  for (unsigned i = 0; i < count; i++) {
    if ((uint8_t)buffer[STATE_HEADER_SIZE + i * STATE_INDENT_SIZE + 4] > 1) return;
  }

  scanner->last_indentation_size = flags & 1 ? (int64_t)indentation : -1;
  scanner->last_column = flags & 2 ? (int64_t)column : -1;
  scanner->last_newline_count = newlines;
  scanner->last_char = character;
  scanner->after_colon_eol = (flags >> 2) & 1;
  array_reserve(&scanner->indents, count);
  for (unsigned i = 0; i < count; i++) {
    const char *entry = buffer + STATE_HEADER_SIZE + i * STATE_INDENT_SIZE;
    array_push(&scanner->indents, (int64_t)read_u32(entry) | ((int64_t)(uint8_t)entry[4] << 32));
  }
}
...
// that is not another `case` clause. The flag sits above the full uint32 lexer column domain.
#define CASE_INDENT_FLAG INT64_C(0x100000000)

static inline int64_t indent_width(int64_t entry) {
  return entry == -1 ? -1 : (entry & UINT32_MAX);
}
...
static inline bool at_case_region_width(int64_t prev, int64_t width) {
...
static bool scan_impl(void *payload, TSLexer *lexer,
  ...
  Scanner *scanner = (Scanner *)payload;
  int64_t prev = scanner->indents.size > 0 ? *array_back(&scanner->indents) : -1;
  int64_t prev_width = indent_width(prev);
  // Layout distinguishes no newline, one newline, and a blank-line separation.
  int64_t newline_count = 0;
  int64_t indentation_size = 0;

  while (is_space(lexer->lookahead)) {
    if (lexer->lookahead == '\n') {
      if (newline_count < 2) newline_count++;
      indentation_size = 0;
    }
    else {
      indentation_size++;
  ...
  if (valid_symbols[OUTDENT] && !valid_symbols[ERROR_SENTINEL] &&
      ...
      (lexer->eof(lexer)
           ? scanner->last_column == -1
           : (int64_t)lexer->get_column(lexer) == scanner->last_column)) {
    lexer->mark_end(lexer);
  ...
  if (
    ...
    switch (check_comment_at_layout(lexer, valid_symbols)) {
      ...
      case COMMENT_SAME_LINE_CODE: {
        ...
        int64_t effective = (int64_t)lexer->get_column(lexer);
    ...
    int64_t entry = indentation_size;
    ...
    if (scanner->indents.size == MAX_INDENTS) return false;
  ...
  if (valid_symbols[OUTDENT] &&
    ...
    if (lexer->eof(lexer)) {
      ...
      scanner->last_column = (int64_t)lexer->get_column(lexer);
      scanner->last_char = lexer->lookahead;
    }
  ...
  if (valid_symbols[INDENT] && !valid_symbols[ERROR_SENTINEL] &&
    ...
    if (is_case_clause_intro(lexer)) {
      if (scanner->indents.size == MAX_INDENTS) return false;
      array_push(&scanner->indents, indentation_size | CASE_INDENT_FLAG);
      lexer->result_symbol = INDENT;

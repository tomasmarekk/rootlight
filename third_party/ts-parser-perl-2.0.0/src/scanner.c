...
// Delimiters are exact UTF-8, bounded by the runtime's existing frame capacity.
// Oversized candidates are consumed but cannot compare equal or enter live state.
#define MAX_TSPSTRING_LEN TREE_SITTER_SERIALIZATION_BUFFER_SIZE
typedef struct {
  uint16_t length;
...
static void tspstring_push(TSPString *s, int32_t c) {
  if (s->length > MAX_TSPSTRING_LEN) return;
  uint8_t encoded[4];
  size_t length;
  if (c >= 0 && c < 0x80) {
    encoded[0] = (uint8_t)c;
    ...
    encoded[0] = 0xc0 | (uint8_t)(c >> 6);
    encoded[1] = 0x80 | (uint8_t)(c & 0x3f);
    length = 2;
  } else if (c < 0x10000 && c >= 0 && !(c >= 0xd800 && c <= 0xdfff)) {
    encoded[0] = 0xe0 | (uint8_t)(c >> 12);
    encoded[1] = 0x80 | (uint8_t)((c >> 6) & 0x3f);
    encoded[2] = 0x80 | (uint8_t)(c & 0x3f);
    length = 3;
  } else if (c >= 0x10000 && c <= 0x10ffff) {
    encoded[0] = 0xf0 | (uint8_t)(c >> 18);
    encoded[1] = 0x80 | (uint8_t)((c >> 12) & 0x3f);
    encoded[2] = 0x80 | (uint8_t)((c >> 6) & 0x3f);
    encoded[3] = 0x80 | (uint8_t)(c & 0x3f);
    length = 4;
  } else {
    s->length = MAX_TSPSTRING_LEN + 1;
    return;
  }
  if (s->length + length > MAX_TSPSTRING_LEN) {
    s->length = MAX_TSPSTRING_LEN + 1;
    return;
  }
  memcpy(s->contents + s->length, encoded, length);
  s->length += (uint16_t)length;
}

static bool tspstring_eq(TSPString *s1, TSPString *s2) {
  return s1->length <= MAX_TSPSTRING_LEN && s1->length == s2->length &&
      memcmp(s1->contents, s2->contents, s1->length) == 0;
}
...
static size_t lexerstate_serialized_size(const LexerState *state) {
  size_t size = 4 + state->quotes.size * sizeof(TSPQuote);
  for (uint8_t i = 0; i < state->heredoc_count; i++)
    size += 4 + state->heredoc_queue[i].delim.length;
  return size;
}

static bool lexerstate_push_quote(LexerState *state, int32_t opener) {
  if (state->quotes.size == UINT8_MAX ||
      lexerstate_serialized_size(state) + sizeof(TSPQuote) > TREE_SITTER_SERIALIZATION_BUFFER_SIZE)
    return false;
...
// Admit only state that can round-trip exactly. Queue and frame exhaustion must
// become parse recovery, never a silently overwritten delimiter or quote stack.
static bool lexerstate_add_heredoc(LexerState *state, TSPString *delim, bool interp, bool indent) {
  if (state->heredoc_count >= HEREDOC_QUEUE_MAX ||
      delim->length > MAX_TSPSTRING_LEN ||
      lexerstate_serialized_size(state) + 4 + delim->length > TREE_SITTER_SERIALIZATION_BUFFER_SIZE)
    return false;
...
// The 1024-byte runtime frame stores every admitted delimiter byte. Validate
// complete frames before publishing state; native struct flags are checked before
// memcpy can create a noncanonical bool. Empty or malformed input resets all state.
unsigned int tree_sitter_perl_external_scanner_serialize(void *payload, char *buffer) {
  LexerState *state = payload;
  size_t size = 0;
  buffer[size++] = (char)state->quotes.size;
  if (state->quotes.size)
    memcpy(buffer + size, state->quotes.contents, state->quotes.size * sizeof(TSPQuote));
  size += state->quotes.size * sizeof(TSPQuote);
  ...
  for (uint8_t i = 0; i < state->heredoc_count; i++) {
    ...
    buffer[size++] = (char)(e->delim.length & 0xff);
    buffer[size++] = (char)(e->delim.length >> 8);
    memcpy(buffer + size, e->delim.contents, e->delim.length);
    size += e->delim.length;
  }
...
void tree_sitter_perl_external_scanner_deserialize(void *payload, const char *buffer,
  ...
  memset(state->heredoc_queue, 0, sizeof(state->heredoc_queue));
  if (length == 0) return;
  if (length < 4 || length > TREE_SITTER_SERIALIZATION_BUFFER_SIZE) return;

  size_t quotes = (uint8_t)buffer[0];
  size_t cursor = 1 + quotes * sizeof(TSPQuote);
  if (cursor + 3 > length) return;
  uint8_t heredoc_state = (uint8_t)buffer[cursor++];
  uint8_t heredocs = (uint8_t)buffer[cursor++];
  if (heredoc_state > HEREDOC_END || heredocs > HEREDOC_QUEUE_MAX ||
      (heredoc_state == HEREDOC_NONE) != (heredocs == 0)) return;
  for (size_t i = 0; i < quotes; i++) {
    size_t start = 1 + i * sizeof(TSPQuote);
    if ((uint8_t)buffer[start + offsetof(TSPQuote, body_leads_with_delim)] > 1) return;
    TSPQuote quote;
    memcpy(&quote, buffer + start, sizeof(quote));
    if (quote.count < 0 || quote.open < 0 || quote.open > 0x10ffff ||
        quote.close <= 0 || quote.close > 0x10ffff) return;
    if (quote.open && close_for_open(quote.open) != quote.close) return;
  }
  for (size_t i = 0; i < heredocs; i++) {
    if (cursor + 4 > length) return;
    if ((uint8_t)buffer[cursor++] > 1 || (uint8_t)buffer[cursor++] > 1) return;
    size_t bytes = (uint8_t)buffer[cursor] | ((size_t)(uint8_t)buffer[cursor + 1] << 8);
    cursor += 2;
    if (bytes > MAX_TSPSTRING_LEN || cursor + bytes > length) return;
    cursor += bytes;
  }
  if (cursor + 1 != length || (uint8_t)buffer[cursor] > 1) return;

  cursor = 1;
  if (quotes) {
    array_reserve(&state->quotes, quotes);
    state->quotes.size = (uint32_t)quotes;
    memcpy(state->quotes.contents, buffer + cursor, quotes * sizeof(TSPQuote));
  }
  cursor += quotes * sizeof(TSPQuote);
  state->heredoc_state = (enum HeredocState)(uint8_t)buffer[cursor++];
  state->heredoc_count = (uint8_t)buffer[cursor++];
  for (uint8_t i = 0; i < state->heredoc_count; i++) {
    HeredocEntry *e = &state->heredoc_queue[i];
    e->interpolates = (bool)buffer[cursor++];
    e->indents = (bool)buffer[cursor++];
    e->delim.length = (uint8_t)buffer[cursor] | ((uint16_t)(uint8_t)buffer[cursor + 1] << 8);
    cursor += 2;
    memcpy(e->delim.contents, buffer + cursor, e->delim.length);
    cursor += e->delim.length;
  }
  state->recovery_emitted = (bool)buffer[cursor];
}
...
bool tree_sitter_perl_external_scanner_scan(void *payload, TSLexer *lexer,
          ...
          if (valid_symbols[TOKEN_OPEN_FILEGLOB_BRACKET]) {
            ...
            if (c == '>') {
              ...
              if (tsp_is_fileglob(content, clen, after, alen)) {
                if (!lexerstate_push_quote(state, '<')) return false;
  ...
  if (valid_symbols[TOKEN_HEREDOC_DELIM] || valid_symbols[TOKEN_COMMAND_HEREDOC_DELIM]) {
    ...
    if (!skipped_whitespace) {
      ...
      if (isidfirst(c)) {
        ...
        if (!lexerstate_add_heredoc(state, &delim, should_interpolate, should_indent)) return false;
  ...
  if (valid_symbols[TOKEN_QUOTELIKE_BEGIN]) {
    ...
    if (!lexerstate_push_quote(state, delim)) return false;

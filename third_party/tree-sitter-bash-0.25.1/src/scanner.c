...
static void truncate_heredocs(Scanner *scanner, uint32_t count) {
    while (scanner->heredocs.size > count) {
...
static void deserialize(Scanner *scanner, const char *buffer, unsigned length) {
    // Validate the whole state before allocating or retaining any prior context.
    if (length < 4 || length > TREE_SITTER_SERIALIZATION_BUFFER_SIZE) {
        reset(scanner);
        return;
    }
    uint32_t cursor = 4;
    uint32_t count = (unsigned char)buffer[3];
    for (uint32_t i = 0; i < count; i++) {
        if (length - cursor < 3 + sizeof(uint32_t)) {
            reset(scanner);
            return;
        }
        cursor += 3;
        uint32_t delimiter_size;
        memcpy(&delimiter_size, &buffer[cursor], sizeof(uint32_t));
        cursor += sizeof(uint32_t);
        if (delimiter_size > length - cursor ||
            (delimiter_size > 0 && buffer[cursor + delimiter_size - 1] != '\0')) {
            reset(scanner);
            return;
        }
    ...
    truncate_heredocs(scanner, count);
    {
        uint32_t size = 0;
...
// TSLexer supplies Unicode scalars, while serialized delimiters contain UTF-8.
static uint32_t encode_scalar(int32_t scalar, char bytes[4]) {
...
static bool advance_word(TSLexer *lexer, String *unquoted_word) {
    ...
    while (lexer->lookahead &&
        ...
        char bytes[4];
        uint32_t count = encode_scalar(lexer->lookahead, bytes);
        if (count == 0) {
            return false;
        }
        empty = false;
        array_extend(unquoted_word, count, bytes);
        advance(lexer);
    }
...
static bool scan_heredoc_end_identifier(Heredoc *heredoc, TSLexer *lexer) {
    ...
    uint32_t size = 0;
    uint32_t delimiter_size = heredoc->delimiter.size - 1;
    while (size < delimiter_size && lexer->lookahead != '\0' && lexer->lookahead != '\n') {
        char bytes[4];
        uint32_t count = encode_scalar(lexer->lookahead, bytes);
        if (count == 0 || count > delimiter_size - size ||
            memcmp(heredoc->delimiter.contents + size, bytes, count) != 0) {
            return false;
        }
        size += count;
        advance(lexer);
    }
    return size == delimiter_size && (lexer->lookahead == '\n' || lexer->eof(lexer));
}

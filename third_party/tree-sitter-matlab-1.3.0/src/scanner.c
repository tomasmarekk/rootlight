...
static inline void consume_identifier(TSLexer* lexer, char* buffer)
{
    ...
    do {
        // Both callers provide 256 bytes for keyword comparison, not token storage.
        // Keep the terminator inside that buffer while consuming the whole source token.
        if (size < 255) {
...
{
    Scanner* scanner = (Scanner*) payload;
    // Empty frames are Tree-sitter resets, not requests to retain the previous parse.
    *scanner = (Scanner) {0};
    if (length == 5 && (unsigned char) buffer[0] <= 1
        && (unsigned char) buffer[1] <= 1 && (unsigned char) buffer[2] <= 1
        && (buffer[3] == 0 || buffer[3] == '\'' || buffer[3] == '"')
...
scan_next:
    ...
    if (percent || line_continuation) {
        ...
        if (lexer->lookahead == '%') {
            // Consecutive comment lines must not consume one native stack frame each.
            entry_delimiter = false;
            ctranspose = false;
            skipped = 0;
            goto scan_next;
        }

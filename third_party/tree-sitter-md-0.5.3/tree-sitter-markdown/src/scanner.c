...
#include "../../common/scanner_state.h"
#include <assert.h>
#include <ctype.h>
#include <string.h>
#include <wchar.h>
...
// Every accepted stack fits the unchanged Tree-sitter serialization buffer.
// Fixed storage also avoids allocation during speculative scans and restoration.
#define STATE_HEADER_SIZE 23u
#define MAX_BLOCKS (TREE_SITTER_SERIALIZATION_BUFFER_SIZE - STATE_HEADER_SIZE)

typedef struct {
    struct {
        uint16_t size;
        uint8_t items[MAX_BLOCKS];
    } open_blocks;
    uint8_t state;
    uint16_t matched;
    // Source coordinates are 32-bit; tab-expanded indentation can be larger.
    uint64_t indentation;
    uint8_t column;
    uint64_t fenced_code_block_delimiter_length;
    bool simulate;
    bool failed;
} Scanner;

static void push_block(Scanner *s, Block b) {
    if (s->open_blocks.size >= MAX_BLOCKS || b < BLOCK_QUOTE || b > ANONYMOUS) {
        s->failed = true;
        return;
    }
    s->open_blocks.items[s->open_blocks.size++] = (uint8_t)b;
}
...
static inline Block pop_block(Scanner *s) {
    ...
    return (Block)s->open_blocks.items[--s->open_blocks.size];
}

static unsigned serialize(Scanner *s, char *buffer) {
    if (s == NULL || s->failed || s->matched > s->open_blocks.size) {
        buffer[0] = (char)0xff;
        return 1;
    }
    buffer[0] = 1;
    buffer[1] = (char)s->state;
    markdown_write_u16(buffer + 2, s->matched);
    markdown_write_u64(buffer + 4, s->indentation);
    buffer[12] = (char)s->column;
    markdown_write_u64(buffer + 13, s->fenced_code_block_delimiter_length);
    markdown_write_u16(buffer + 21, s->open_blocks.size);
    memcpy(buffer + STATE_HEADER_SIZE, s->open_blocks.items, s->open_blocks.size);
    return STATE_HEADER_SIZE + s->open_blocks.size;
}
...
static void deserialize(Scanner *s, const char *buffer, unsigned length) {
    ...
    // Failed or malformed snapshots must not resume as a successful empty stack.
    s->failed = true;
    if (buffer == NULL || length < STATE_HEADER_SIZE ||
        length > TREE_SITTER_SERIALIZATION_BUFFER_SIZE ||
        (uint8_t)buffer[0] != 1) return;
    const uint16_t count = markdown_read_u16(buffer + 21);
    const uint16_t matched = markdown_read_u16(buffer + 2);
    const uint8_t state = (uint8_t)buffer[1];
    const uint8_t column = (uint8_t)buffer[12];
    const uint64_t indentation = markdown_read_u64(buffer + 4);
    const uint64_t fence = markdown_read_u64(buffer + 13);
    if (length != STATE_HEADER_SIZE + count || count > MAX_BLOCKS ||
        matched > count || (state & (uint8_t)~0x13u) != 0 || column > 3 ||
        indentation > (uint64_t)UINT32_MAX * 4 || fence > UINT32_MAX) return;
    for (unsigned i = 0; i < count; i++) {
        if ((uint8_t)buffer[STATE_HEADER_SIZE + i] > ANONYMOUS) return;
    }
    s->state = state;
    s->matched = matched;
    s->indentation = indentation;
    s->column = column;
    s->fenced_code_block_delimiter_length = fence;
    memcpy(s->open_blocks.items, buffer + STATE_HEADER_SIZE, count);
    s->open_blocks.size = count;
    s->failed = false;
}
...
static bool scan(Scanner *s, TSLexer *lexer, const bool *valid_symbols) {
    ...
    if (!(s->state & STATE_MATCHING)) {
        ...
        while (s->matched < s->open_blocks.size) {
            if (s->matched == s->open_blocks.size - 1 &&
                (s->state & STATE_CLOSE_BLOCK)) {
                if (!partial_success)
                    s->state &= ~STATE_CLOSE_BLOCK;
                break;
    ...
    if ((valid_symbols[LINE_ENDING] || valid_symbols[SOFT_LINE_ENDING] ||
        ...
        if (!(s->state & STATE_CLOSE_BLOCK) &&
            ...
            s->simulate = true;
            uint16_t matched_temp = s->matched;
            s->matched = 0;
...
bool tree_sitter_markdown_external_scanner_scan(void *payload, TSLexer *lexer,
    ...
    if (scanner == NULL || scanner->failed) return error(lexer);
    scanner->simulate = false;
    bool found = scan(scanner, lexer, valid_symbols);
    return scanner->failed ? error(lexer) : found;
}

...
typedef struct {
    // Parser state flags
    uint8_t state;
    uint64_t code_span_delimiter_length;
    uint64_t latex_span_delimiter_length;
    // The number of characters remaining in the currrent emphasis delimiter
    // run.
    uint64_t num_emphasis_delimiters_left;

    bool failed;
} Scanner;

// Complete, versioned snapshots preserve wide delimiter runs on every platform.
static unsigned serialize(Scanner *s, char *buffer) {
    ...
    markdown_write_u64(buffer + 2, s->code_span_delimiter_length);
    markdown_write_u64(buffer + 10, s->latex_span_delimiter_length);
    markdown_write_u64(buffer + 18, s->num_emphasis_delimiters_left);
    return 26;
}
...
static void deserialize(Scanner *s, const char *buffer, unsigned length) {
    ...
    if (length == 0) return;
    s->failed = true;
    if (buffer == NULL || length != 26 || (uint8_t)buffer[0] != 1 ||
        ((uint8_t)buffer[1] & (uint8_t)~STATE_EMPHASIS_DELIMITER_IS_OPEN) != 0) return;
    const uint64_t code_length = markdown_read_u64(buffer + 2);
    const uint64_t latex_length = markdown_read_u64(buffer + 10);
    const uint64_t emphasis_left = markdown_read_u64(buffer + 18);
    if (code_length > UINT32_MAX || latex_length > UINT32_MAX ||
        emphasis_left > UINT32_MAX) return;
    s->state = (uint8_t)buffer[1];
    s->code_span_delimiter_length = code_length;
    s->latex_span_delimiter_length = latex_length;
    s->num_emphasis_delimiters_left = emphasis_left;
    s->failed = false;
}

static bool parse_leaf_delimiter(TSLexer *lexer, uint64_t *delimiter_length,

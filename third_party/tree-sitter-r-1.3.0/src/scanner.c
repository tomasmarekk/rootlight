// R raw-string and contextual-newline state for the generated parser.
// The local boundary checks are documented in ../PATCHES.md.
...
static bool raw_string_state_deserialize(RawStringState* raw_string_state, DeserializeBuffer* buffer) {
  ...
  // Only the reset tuple or a complete delimiter can enter persistent state.
  if (raw_string_state->closing_bracket == '\0') {
    return raw_string_state->hyphen_count == 0 && raw_string_state->closing_quote == '\0';
  }
  return (raw_string_state->closing_bracket == ')' || raw_string_state->closing_bracket == ']' ||
          raw_string_state->closing_bracket == '}') &&
         (raw_string_state->closing_quote == '"' || raw_string_state->closing_quote == '\'');
}
...
static bool scopes_pop(Scopes* scopes, Scope expected) {
  ...
  // A rejected closing token must not remove the still-open scope.
  scopes->count--;
  return true;
}
...
static bool scopes_deserialize(Scopes* scopes, DeserializeBuffer* buffer) {
  ...
  // Validate capacity and every scope before copying an attacker-controlled frame.
  if (scopes->count > MAX_SCOPES_COUNT || buffer->length != scopes->count * SCOPE_SIZE) {
    return false;
  }
  for (unsigned i = 0; i < scopes->count; ++i) {
    if (buffer->pointer[i] < SCOPE_BRACE || buffer->pointer[i] > SCOPE_BRACKET2) {
      return false;
    }
  }
...
static inline bool is_scanner_space(int32_t point) {
  // Windows wint_t cannot represent supplementary Unicode code points.
  return point >= 0 && (uint32_t)point <= WCHAR_MAX && iswspace((wint_t)point);
}

static inline void consume_whitespace_and_ignored_newlines(TSLexer* lexer, Scopes* scopes) {
  while (is_scanner_space(lexer->lookahead)) {
    if (lexer->lookahead != '\n') {
...
static inline bool is_identifier_continuation(int x) {
  return x >= 128 || x == '_' || x == '.' || (x >= 0 && iswalnum((wint_t)x));
}
...
static inline bool scan_raw_string_open(TSLexer* lexer, RawStringState* raw_string_state) {
  ...
  int32_t closing_quote = lexer->lookahead;
  ...
  int32_t opening_bracket = lexer->lookahead;
  char closing_bracket;
  if (opening_bracket == '(') {
    closing_bracket = ')';
    lexer->advance(lexer, false);
  ...
  // The quote was validated as ASCII before narrowing the full lexer code point.
  raw_string_state_set(raw_string_state, closing_bracket, hyphen_count, (char)closing_quote);
  return true;
}
...
// Revalidate after incremental edits: restored state does not validate new source bytes.
static inline bool scan_raw_string_close(TSLexer* lexer, RawStringState* raw_string_state) {
  if (lexer->eof(lexer) || lexer->lookahead != raw_string_state->closing_bracket) {
    return false;
  }
  ...
  if (lexer->eof(lexer) || lexer->lookahead != raw_string_state->closing_quote) {
    return false;
  }
...
void tree_sitter_r_external_scanner_deserialize(void* payload, const char* buffer, unsigned length) {
  ...
  if (length == 0 || length > TREE_SITTER_SERIALIZATION_BUFFER_SIZE || buffer == NULL) {
    // Zero length resets a parse; malformed frames must not retain any prior context.

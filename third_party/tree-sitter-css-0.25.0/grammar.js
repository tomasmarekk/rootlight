...
export default grammar({
  ...
  rules: {
    ...
    class_name: $ => seq(
      choice(alias($._unescaped_identifier, $.identifier), alias($._identifier_escape, $.escape_sequence)),
      repeat(choice(
        alias(/[a-zA-Z0-9_\-\u0080-\u{10FFFF}]+/u, $.identifier),
        alias($._identifier_escape, $.escape_sequence),
      )),
    ),

    identifier: _ => token(identifierPattern()),

    _identifier_escape: _ => token(identifierEscapePattern()),

    // Class selectors keep their established explicit escape child nodes.
    _unescaped_identifier: _ => /(--|-?[a-zA-Z_\u0080-\u{10FFFF}])[a-zA-Z0-9_\-\u0080-\u{10FFFF}]*/u,

    at_keyword: _ => token(seq('@', identifierPattern())),
...
// CSS identifiers admit every non-ASCII scalar and valid escapes, not just
// Latin-1 letters. Hex escape terminators consume one CSS whitespace unit.
function identifierEscapePattern() {
  return seq('\\', choice(
    seq(/[0-9a-fA-F]{1,6}/, optional(choice('\r\n', /[ \t\n\r\f]/))),
    /[^0-9a-fA-F\n\r\f]/,
  ));
}

function identifierPattern() {
  const escape = identifierEscapePattern();
  return seq(
    choice('--', seq(optional('-'), choice(/[a-zA-Z_\u0080-\u{10FFFF}]/u, escape))),
    repeat(choice(/[a-zA-Z0-9_\-\u0080-\u{10FFFF}]/u, escape)),
  );
}
